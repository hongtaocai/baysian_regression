...
class Chbtc(Exchange):
    ...
    def get_trade(self, ticker, since = None):
        url = self.trade_url
        if ticker == 'ltc_cny':
            url = 'http://api.chbtc.com/data/ltc/getTrades'
        if since == None:
            response = self.get_data(url)
        else:
        ...
        if len(data) == 0 or (len(data) == 1 and data[0] == []):
            ...
            if type(data) is dict:
                data = [data] 
            since = max([int(data[i]['tid']) for i in range(len(data))])
            for trade in data:
                if trade['type'] == 'buy':
                    trade['type'] = 'bid'
            ...
            query = '''INSERT IGNORE INTO chbtc_trade (ticker, id, timestamp, price, type, amount) VALUES ''' + ','.join(['''('{}', {}, {}, {}, '{}', {}) '''] * (len(data)))
